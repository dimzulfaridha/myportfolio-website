...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 selection:bg-accent/30 font-sans pb-4">
      ...
      {/* Floating Navigation */}
      <motion.nav
        className={`fixed left-0 right-0 z-50 flex justify-center transition-all duration-700 ease-out ${scrolled ? "top-4 px-4 md:top-6" : "top-0 px-4 pt-4 md:top-8 md:px-4"}`}
      >
        <motion.div
          layout
          transition={{ type: "spring", stiffness: 200, damping: 25 }}
          className={`flex flex-col w-full transition-all duration-700 ease-out ${scrolled
        ...
        >
          <div className={`flex justify-between items-center w-full gap-4 transition-all duration-700 ease-out ${scrolled ? "px-5 py-3 md:px-6 md:py-3" : "px-0 py-2 md:px-2 md:py-2"}`}>
